...
// 从控制器获取标题
$pageTitle = $data['title'] ?? LanguageManager::t('Monthly Report');
...
$content = (new CDiv())
    ...
    ->addItem(
        (new CTag('h1', true, LanguageManager::t('Zabbix Monthly Report') . ' - ' . $data['report_period']))
    )
    ->addItem(
        (new CDiv())
            ...
            ->addItem(
                (new CButton('export_pdf', LanguageManager::t('Export PDF')))
                    ->onClick('javascript: window.open("?action=reports.monthly.export&format=pdf", "_blank");')
            )
            ->addItem(
