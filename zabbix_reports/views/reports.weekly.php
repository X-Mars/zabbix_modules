...
$page
    ->setTitle(LanguageManager::t('Weekly Report'))
    ->addItem(
        (new CDiv())
            ->addClass('report-container')
            ->addItem(
                (new CTag('h1', true, LanguageManager::t('Zabbix Weekly Report') . ' - ' . $data['report_period']))
            )
            ->addItem(
                (new CDiv())
                    ...
                    ->addItem(
                        (new CButton('export_pdf', LanguageManager::t('Export PDF')))
                            ->onClick('javascript: window.open("?action=reports.weekly.export&format=pdf", "_blank");')
                    )
                    ->addItem(
